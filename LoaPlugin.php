...
class LoaPlugin extends GenericPlugin
{
    ...
    public function generateLoaHtml($submission)
    {
        ...
        $headerTpl = '<p style="text-align: center;">{$journalLogo}</p><p style="text-align: center;"><span style="font-size: 18pt;"><strong>LETTER OF ACCEPTANCE</strong></span><br><strong>{$journalTitle}</strong></p>';
        $footerTpl = '<p>Sincerely,</p><p><strong>Editor in Chief</strong></p><p><img src="{$qrCodeDataUri}" alt="QR Code" width="90"></p><p><strong>Dr. Pribadi</strong><br>{$journalTitle}</p>';
